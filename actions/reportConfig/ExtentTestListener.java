 ...
 * getTest().log(Status.PASS, MarkupHelper.createLabel(iTestResult.getName() +
 ...
 * driver).getScreenshotAs(OutputType.BASE64); getTest().log(Status.FAIL,
 * "Screenshot and Exception", iTestResult.getThrowable(),
 * getTest().addScreenCaptureFromBase64String(base64Screenshot).getModel().
 * getMedia().get(0)); getTest().log(Status.FAIL,
 * MarkupHelper.createLabel(iTestResult.getName() + " - Failed",
 * ExtentColor.RED)); }
 * 
 * @Override public void onTestSkipped(ITestResult iTestResult) {
 * getTest().log(Status.SKIP, MarkupHelper.createLabel(iTestResult.getName() +
 * " - Skipped", ExtentColor.ORANGE)); }
 * 
 * @Override public void onTestFailedButWithinSuccessPercentage(ITestResult
 * iTestResult) { getTest().log(Status.FAIL,
 * MarkupHelper.createLabel(iTestResult.getName() + " - Failed with Percentage",
 * ExtentColor.RED)); } }
 */